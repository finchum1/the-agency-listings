    ...
    <section className="px-6 lg:px-10 py-24">
      <div className="mx-auto max-w-6xl grid lg:grid-cols-3 gap-14">
        <div className="lg:col-span-2">
          ...
          <div className="space-y-5 text-[#1c1a17]/75 leading-relaxed text-[15.5px]">
        ...
        <div className="bg-white rounded-2xl border border-black/5 p-8 h-fit">
          <h3 className="text-sm font-semibold tracking-wider-plus uppercase text-[#1c1a17]/60 mb-5">
            Quick Facts
          </h3>
          <dl className="space-y-4">
            {facts.map((f) => (
              <div key={f.label} className="flex items-center justify-between text-sm border-b border-black/5 pb-3 last:border-0 last:pb-0">
                <dt className="text-[#1c1a17]/55">{f.label}</dt>
                <dd className="font-medium text-[#1c1a17]">{f.value}</dd>
              </div>
            ))}
          </dl>
        </div>
