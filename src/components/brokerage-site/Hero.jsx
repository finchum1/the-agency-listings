...
export default function Hero() {
  ...
  return (
    <section className="relative h-screen min-h-[640px] w-full">
      {site.heroVideo ? (
        <video className="absolute inset-0 h-full w-full object-cover" src={site.heroVideo} poster={site.heroPhoto} autoPlay muted loop playsInline />
      ) : site.heroPhoto ? (
      ...
      {/* Hardcoded white, not --as-on-dark: that token means "text on
          --as-dark" (a solid band like the footer/nav), which for most
          templates happens to be light anyway -- but the Linen template
          flips --as-on-dark to dark ink, and this text always sits on
          the black photo overlay above, not on --as-dark itself, so it
          needs to stay white regardless of theme. */}
      <div className="relative z-10 flex h-full flex-col justify-end px-6 lg:px-10 pb-24 max-w-7xl mx-auto">
        <p className="text-white/75 text-xs tracked-wide uppercase mb-4">Oklahoma</p>
