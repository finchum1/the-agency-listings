...
const PAGES = [
  ...
  { path: "/brokerage/blog", label: "Blog" },
];

// Parallel to agent-site/Navbar.jsx — fixed nav (no per-slug SiteLink
// needed, there's only one brokerage site) and no home_sections toggle,
// since the brokerage site's pages aren't user-configurable.
    ...
    >
      <div className="mx-auto max-w-7xl px-6 lg:px-10 flex items-center justify-between py-4">
        <Link to="/brokerage" className="flex items-center gap-3 min-w-0">
          <img src={site.brokerage.logos.white} alt={site.brokerage.name} className="h-9 sm:h-11 w-auto" />
        </Link>
