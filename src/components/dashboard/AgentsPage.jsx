...
export default function AgentsPage() {
  ...
  const handleInvite = async (e) => {
    ...
    try {
      ...
      setSuccess(`Invited ${form.email}.`);
      setForm({ email: "", full_name: "", title: "Real Estate Agent", license: "", phone: "", role: "agent" });
      refresh();
    } catch (err) {
  ...
  return (
    <div className="grid lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white border border-black/5 rounded-2xl divide-y divide-black/5">
            {agents.map((a) => (
              <div key={a.id} className="px-5 py-4 flex items-center justify-between">
                <div>
                  <p className="font-medium text-sm">{a.full_name || "(no name yet)"}</p>
                  <p className="text-xs text-[#1c1a17]/50">{a.email}</p>
                </div>
                <span
      ...
      <div>
        <h2 className="text-lg font-display font-semibold mb-4 mt-1">Invite an Agent</h2>
        <form onSubmit={handleInvite} className="bg-white border border-black/5 rounded-2xl p-6 space-y-4">
