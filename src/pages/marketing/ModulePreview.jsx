...
// TEMPORARY — a screenshot-capture route for the /upcoming marketing
// page's three BrowserFrame images (a distinct hero composite plus one
// full table per tool). Renders the REAL table components (see
...
//
// Capture notes: the tables use whitespace-nowrap columns and don't
// reflow at different viewport widths (only horizontally scroll via
// overflow-x-auto below their natural ~1200px content width), so a
// narrower capture viewport wouldn't shrink the image, it would just
// clip it. The real legibility fix belongs on the marketing page's own
// layout (display these wide, not squeezed into a half-width grid
// column) — this file's only job is a clean, unclipped capture with
// real padding so a heading doesn't touch the top edge.
export default function ModulePreview() {
  const noop = () => {};
  return (
    <div className="min-h-screen bg-[#faf9f7]">
      <div className="space-y-16 py-16">
        {/* Hero composite — deliberately NOT identical to either full-table
            shot below: two rows each, stacked, so it reads as "a glance at
            both tools" rather than a duplicate of the Upcoming Listings
            section right underneath it on the real page. */}
        <div id="hero-shot" className="bg-[#faf9f7] p-8 inline-block space-y-8">
          <div>
            <h2 className="text-lg font-display font-semibold mb-3">Upcoming Listings</h2>
            <UpcomingListingsTable
              rows={SAMPLE_UPCOMING.slice(0, 2)}
          ...
          <div>
            <h2 className="text-lg font-display font-semibold mb-3">Buyer Needs</h2>
            <BuyerNeedsTable
              rows={SAMPLE_BUYERS.slice(0, 2)}
        ...
        <div id="upcoming-listings-shot" className="bg-[#faf9f7] p-8 inline-block">
        ...
        <div id="buyer-needs-shot" className="bg-[#faf9f7] p-8 inline-block">
