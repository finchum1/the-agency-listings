...
### Marketing pages (`src/pages/LandingPage.jsx`, `AgentWebsitesPage.jsx`, `PropertyWebsitesPage.jsx`, `BrokerageWebsitePage.jsx`)
Four pages sharing one set of chrome components (`src/components/marketing/`: `MarketingNav.jsx`, `MarketingFooter.jsx`, `DeviceFrames.jsx`, `motion.jsx` — the `Reveal`/`variants`/`easeOut` entrance language, moved out of `LandingPage.jsx` so all four pages reuse the exact same motion): the home page gives a short highlight of each product (Agent Websites, Property Websites, Brokerage Site) with a link to that product's own deep-dive page, which carries the full pitch — hero, capability sections, a mobile-device proof (where relevant), and a closing CTA. Sign-up is invite-only (see PRODUCT.md), so the three deep-dive pages' CTAs are a `mailto:` ("Get in Touch," via `src/lib/marketingContact.js`) rather than "Sign In" — "Sign In" stays reserved for the home page's own CTAs, aimed at agents who already have an account. `BrokerageWebsitePage.jsx` pitches the one office-wide site (`/brokerage`) — full reciprocal IDX Home Search (map + list, draw-a-boundary), the office's own Our Listings, and the Home Valuation lead form — using real screenshots of the live site even though it (like all Repliers-backed pages, pending real MLSOK vendor approval) is currently showing Repliers sandbox listings rather than live Oklahoma inventory; the search mechanism being shown is real regardless of the underlying data.

The landing page also carries one non-product section — dark mode + PWA installability — styled lighter than the three product highlights (a single screenshot, no device-frame pair) since it's a quality-of-life improvement to the existing dashboard, not a fourth product with its own deep-dive page.

**Dashboard dark mode** (`src/hooks/useTheme.js`, a `dark` class on `<html>` gating Tailwind's `dark:` variant, `@custom-variant dark` in `src/index.css`): a light/dark/system toggle for the *dashboard only* (Settings → Appearance), layered on top of this same cream/ink/brand-red system rather than a new one — dark values were added inline alongside each existing light-mode utility class (e.g. `bg-white dark:bg-[#1a1a1a]`), not a parallel token system. Dark surfaces are neutral near-black (`#0d0d0d` page / `#1a1a1a` cards), not a warm/brown dark — deliberately distinct from the warm ink (`#1c1a17`) used everywhere else in this system, since ink read as brown once actually rendered as a full-screen dark background. Covers the dashboard shell plus every screen agents and admins use (Listings, My Site, Upcoming, Profile/Settings, listing forms, Agents, Sites, the Brokerage Site editor, flyers); does **not** apply to any public-facing page (marketing site, agent/brokerage/listing sites) — those stay on their own existing theme systems regardless of a visitor's dashboard preference. The printable flyer sheet (`FlyerPage.jsx`'s `#flyer-sheet`) is deliberately exempt even within the dashboard, since it's a fixed-appearance print artifact, not dashboard chrome.
...
**Rich text** (Bio, Testimonials, Areas of Expertise, Blog posts): all four are edited through the same `RichTextEditor.jsx` (TipTap), a deliberately narrow WYSIWYG — paragraph, one heading level (`h3`), bold, italic, underline. No links, no colors, no lists — this is short-form marketing copy, not a document editor. Storage/render split:
- **Bio** (`agent_sites.bio_html`) and **blog body** (`agent_site_posts.body_html`) are new columns, backfilled from the old `bio text[]` / `body jsonb` formats by `supabase/rich-text-fields.sql`; the old columns stay in place, deprecated and unused going forward. `adaptAgentSite.js`/`PostsManager.jsx`/`AgentPostPage.jsx` all fall back to converting the old format on the fly (`lib/richTextFallback.js`) for any row the backfill hasn't reached — zero visual change either way.
- **Testimonial quote** and **area description** needed no new column — both were already single plain-text fields, and plain text with no angle brackets renders identically as `{text}` or as sanitized HTML.
- Every public render goes through `lib/sanitizeHtml.js` (DOMPurify, `ALLOWED_TAGS = [p, h3, strong, em, u, br]`) — RLS only restricts *who* can write these columns, not *what*, so sanitizing on read (not just trusting the editor's own output) is what actually closes the XSS gap. Public rendering uses the shared `.rich-text` CSS class (`src/index.css`) so an in-copy `h3` matches that site's own `--font-display`.
- **Areas of Expertise `description` is edited but not currently displayed anywhere on the public site** — `ServiceAreas.jsx` only ever shows `blurb`. Pre-existing gap, not something this feature fixed; worth a dedicated area-detail page if that content should actually surface somewhere.
