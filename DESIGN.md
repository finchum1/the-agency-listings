...
**Logo** (`logo_variant` column — `red` (default) / `white` / `black` / `square`): which official logo shows in `Navbar.jsx`/`Footer.jsx`. Red/white/black are the same wide mark+wordmark lockup recolored (`public/images/brokerage-logo{,-white,-black}.png`, pixel-identical shape, alpha-preserving PIL recolor, not redrawn). Square is a distinct composition — the mark stacked above the wordmark, both white, on a solid brand-red (`#ed2127`) tile (`public/images/brokerage-logo-square.png`) — built from the same two source elements (mark, wordmark) extracted and recomposed, not a separate design. Resolved in `adaptAgentSite.js` (`brokerage.logos[site.logo_variant]`); every other context (dashboard, login, listing sites, flyers) always uses the plain red wide default from `brokerage.js` — only the agent-site header/footer offer a choice.
